...
class LocalCalendarProvider implements CalendarProvider {
  ...
  SolarTermInternalFeatureFlag _solarTermFlag =
      SolarTermInternalFeatureFlag.defaultProduction;
  SolarTermPublicFeatureFlag _publicFlag =
      SolarTermPublicFeatureFlag.fullEnabled;

  void setSolarTermFlag(SolarTermInternalFeatureFlag flag) {
    _solarTermFlag = flag;
  }

  SolarTermInternalFeatureFlag get solarTermFlag => _solarTermFlag;

  void setPublicFlag(SolarTermPublicFeatureFlag flag) {
    _publicFlag = flag;
  }
  ...
  GanzhiInternalFeatureFlag _ganzhiFlag =
      GanzhiInternalFeatureFlag.defaultDisabled;

  void setGanzhiFlag(GanzhiInternalFeatureFlag flag) {
    _ganzhiFlag = flag;
  }

  GanzhiInternalFeatureFlag get ganzhiFlag => _ganzhiFlag;

  GanzhiPublicFeatureFlag _ganzhiPublicFlag =
      GanzhiPublicFeatureFlag.fullEnabled;
  void setGanzhiPublicFlag(GanzhiPublicFeatureFlag flag) {
    _ganzhiPublicFlag = flag;
  }

  GanzhiPublicFeatureFlag get ganzhiPublicFlag => _ganzhiPublicFlag;

  final DayGanzhiInternalEngine _dayGanzhiEngine =
      const DayGanzhiInternalEngine();
  final ClashInternalEngine _clashEngine = const ClashInternalEngine();
  DayGanzhiInternalFeatureFlag _dayGanzhiClashFlag =
      DayGanzhiInternalFeatureFlag.defaultDisabled;

  void setDayGanzhiClashFlag(DayGanzhiInternalFeatureFlag flag) {
    _dayGanzhiClashFlag = flag;
  }

  DayGanzhiInternalFeatureFlag get dayGanzhiClashFlag => _dayGanzhiClashFlag;

  DayGanzhiClashPublicFeatureFlag _dayGanzhiClashPublicFlag =
      DayGanzhiClashPublicFeatureFlag.fullEnabled;
  void setDayGanzhiClashPublicFlag(DayGanzhiClashPublicFeatureFlag flag) {
    _dayGanzhiClashPublicFlag = flag;
  }

  DayGanzhiClashPublicFeatureFlag get dayGanzhiClashPublicFlag =>
      _dayGanzhiClashPublicFlag;
  ...
  CalendarProviderCapabilities getCapabilities() =>
      CalendarProviderCapabilities(
        lunarDate: 'full',
        yearGanzhi: (_ganzhiPublicFlag.ganzhiPublicEnabled &&
                _ganzhiPublicFlag.yearGanzhiEnabled &&
                _ganzhiPublicFlag.sourceIsSafe)
            ? 'full'
            : (_ganzhiFlag.internalGanzhiAllowed ? 'internal' : 'unavailable'),
        monthGanzhi: (_ganzhiPublicFlag.ganzhiPublicEnabled &&
                _ganzhiPublicFlag.monthGanzhiEnabled &&
                _ganzhiPublicFlag.sourceIsSafe)
            ? 'full'
            : (_ganzhiFlag.internalGanzhiAllowed ? 'internal' : 'unavailable'),
        dayGanzhi: (_dayGanzhiClashPublicFlag.dayGanzhiPublicEnabled &&
                _dayGanzhiClashPublicFlag.sourceIsSafe)
            ? 'full'
            : (_dayGanzhiClashFlag.internalDayGanzhiAllowed
                ? 'internal'
                : 'unavailable'),
        zodiac: 'full',
        solarTerm:
            (_publicFlag.solarTermPublicEnabled && _publicFlag.sourceIsSafe)
                ? 'full'
                : (_solarTermFlag.internalSolarTermAllowed
                    ? 'internal'
                    : 'unavailable'),
        clash: (_dayGanzhiClashPublicFlag.clashPublicEnabled &&
                _dayGanzhiClashPublicFlag.sourceIsSafe)
            ? 'full'
            : (_dayGanzhiClashFlag.internalClashAllowed
                ? 'internal'
                : 'unavailable'),
        supportedDateRange: const {'start': '1900-01-01', 'end': '2100-12-31'},
        source: 'local_calendar_provider_v0_45',
        notes: [
          if (_solarTermFlag.internalSolarTermAllowed)
            'v0.45 节气 internal 受控可用（debug/internal only）',
          '干支、冲煞仍未启用',
          '分享、snapshot 仍不含节气',
        ],
      );
  ...
  CalendarDayInfo getDayInfo(DateTime date,
    ...
    final lunar = lunarResult != null
        ? LunarDate(
            ...
            displayText: lunarResult.displayText)
        : LunarDate.unavailable;

    final zodiac = lunar.available
        ? ZodiacInfo(
            available: true,
            zodiac: _computeZodiac(lunar.lunarYear) ?? '',
            lunarYear: lunar.lunarYear,
            basis: 'derived_from_lunar_year',
            displayText: _computeZodiac(lunar.lunarYear) ?? '',
    ...
    final usePublic =
        _ganzhiPublicFlag.ganzhiPublicEnabled && _ganzhiPublicFlag.sourceIsSafe;
    final useInternal =
        _ganzhiFlag.internalGanzhiAllowed && _ganzhiFlag.sourceIsSafe;
    final useDayGanzhi = (_dayGanzhiClashPublicFlag.dayGanzhiPublicEnabled &&
            _dayGanzhiClashPublicFlag.sourceIsSafe) ||
        (_dayGanzhiClashFlag.internalDayGanzhiAllowed &&
            _dayGanzhiClashFlag.sourceIsSafe);
    final useClash = (_dayGanzhiClashPublicFlag.clashPublicEnabled &&
            _dayGanzhiClashPublicFlag.sourceIsSafe) ||
        (_dayGanzhiClashFlag.internalClashAllowed &&
            _dayGanzhiClashFlag.sourceIsSafe);
    ...
    if (usePublic || useInternal) {
      final solarAvailable = _publicFlag.solarTermPublicEnabled ||
          _solarTermFlag.internalSolarTermAllowed;
      final gzResult =
          _ganzhiEngine.compute(date, solarTermAvailable: solarAvailable);
      final yearOk =
          gzResult.ganzhiYear != null && gzResult.ganzhiYear!.isNotEmpty;
      final monthOk =
          gzResult.ganzhiMonth != null && gzResult.ganzhiMonth!.isNotEmpty;
      final displayYear =
          (usePublic && _ganzhiPublicFlag.yearGanzhiEnabled) || (useInternal);
      final displayMonth =
          (usePublic && _ganzhiPublicFlag.monthGanzhiEnabled) || (useInternal);
    ...
    if (useClash && dtext.isNotEmpty) {
      ...
      if (cResult.clashZodiac != null) {
        clash = ClashInfo(
            available: true,
            dayBranch: cResult.dayBranch ?? '',
            clashZodiac: cResult.clashZodiac ?? '',
            shaDirection: cResult.shaDirection ?? '',
            displayText:
                '冲${cResult.clashZodiac} 煞${cResult.shaDirection}'.trim());
      }
    }
    ...
    if (ytext.isNotEmpty || mtext.isNotEmpty || dtext.isNotEmpty) {
      ganzhi = GanzhiInfo(
          ...
          monthGanzhi: mtext,
          dayGanzhi: dtext,
          displayText: '$ytext $mtext $dtext'.trim(),
          basis: basis);
    }
    ...
    return CalendarDayInfo(
      ...
      clash: clash,
      dataQuality: {
        'lunar': lunar.available ? 'full' : 'unavailable',
        'zodiac': zodiac.available ? 'full' : 'unavailable',
        'ganzhi': 'unavailable',
        'solarTerm': _solarTermFlag.internalSolarTermAllowed
            ? 'internal'
            : 'unavailable',
        'clash': 'unavailable'
      },
