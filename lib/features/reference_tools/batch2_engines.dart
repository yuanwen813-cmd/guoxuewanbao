    ...
    return GuoxueResult(
      ...
      sections: [
        ...
        ResultSection(
            ...
            text: '${total.toStringAsFixed(1)} 两'),
...
class ShengXiaoEngine {
  ...
  GuoxueResult calculate(int shengxiaoIndex) {
    ...
    return GuoxueResult(
      ...
      sections: [
        ResultSection(
            title: '生肖', type: ResultSectionType.text, text: '属$name'),
        ResultSection(
            title: '年度总运', type: ResultSectionType.text, text: _overview[i]),
  ...
  GuoxueResult calculate(String dreamText) {
    ...
    if (matches.isEmpty) {
      return GuoxueResult(
        ...
        sections: [
          ResultSection(
              title: '梦境', type: ResultSectionType.text, text: dreamText),
          ResultSection(
              title: '解梦结果',
              type: ResultSectionType.text,
              text:
    ...
    return GuoxueResult(
      ...
      sections: [
        ...
        ResultSection(
            title: '匹配关键词',
            type: ResultSectionType.tags,
            tags: matches.map((m) => m.key).toList()),
      ],
      rawData: {
        'dream': dreamText,
        'matches':
            matches.map((m) => {'keyword': m.key, 'meaning': m.value}).toList()
      },
    );
  }
}
...
class WuXingCalculatorEngine {
  GuoxueResult calculate(String? element) {
    ...
    final selected = element != null
        ? wuxing
            .cast<WuXing?>()
            .firstWhere((w) => w?.chinese == element, orElse: () => null)
        : null;
...
class ErShiBaXiuEngine {
  ...
  GuoxueResult calculate() {
    ...
    return GuoxueResult(
      ...
      sections: [
        ResultSection(
            title: '四象二十八宿', type: ResultSectionType.table, table: rows),
        ResultSection(
            title: '说明',
            type: ResultSectionType.text,
...
class GanZhiCalendarEngine {
  GuoxueResult calculate(DateTime date) {
    ...
    final dayGz = GanZhi(
        TianGan.fromOrder((days + 10) % 10), DiZhi.fromOrder((days + 12) % 12));
    final yearGz = GanZhi(TianGan.fromOrder((date.year - 4) % 10),
        DiZhi.fromOrder((date.year - 4) % 12));
    ...
    return GuoxueResult(
      ...
      sections: [
        ...
        ResultSection(
            ...
            type: ResultSectionType.text,
            text:
                '日柱序号：${((dayGz.tianGan.order * 6 - dayGz.diZhi.order * 5) % 60 + 60) % 60 + 1} / 60'),
      ...
      rawData: {
        ...
        'year': yearGz.chineseName,
        'month': monthGz.chineseName,
        'day': dayGz.chineseName
      },
    );
  }
